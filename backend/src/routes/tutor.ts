...
const llmSchema = z.object({
  mode: z.enum(["local-gguf", "ollama", "groq", "openai"]).optional(),
...
const reviewSchema = z.object({ pseudocode: z.string().trim().min(20, "Describe at least a few steps of your approach.").max(8000) });
...
export function createTutorRouter(store = new SessionStore()): ExpressRouter {
  ...
  router.post("/sessions", createLimiter, openAiMinuteQuota, openAiDailyQuota, async (request, response, next) => {
    try {
      ...
      const state = await createTutorState(body.problemUrl, normalizeLlmSettings(body.llm), body.problemStatement);
  ...
  router.post("/sessions/:id/review", reviewLimiter, openAiMinuteQuota, openAiDailyQuota, async (request, response, next) => {
    try {
      ...
      const state = await reviewTutorState(current.state, body.pseudocode);
