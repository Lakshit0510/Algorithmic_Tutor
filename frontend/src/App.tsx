...
export default function App() {
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-100">
    ...
    <section className="mx-auto max-w-[1600px] px-5 pb-5 md:px-8"><form className="statement-form" onSubmit={start}><div className="url-form"><input type="url" inputMode="url" autoFocus aria-label="Codeforces problem URL" value={url} onInput={(event) => setUrl(event.currentTarget.value)} onChange={(event) => setUrl(event.target.value)} placeholder="Paste a Codeforces problem URL — e.g. codeforces.com/problemset/problem/4/A" required /><button type="submit" disabled={startPending}>{startPending ? "Loading…" : "Start mentoring"}</button></div><label className="statement-fallback">Problem statement fallback <span>Optional unless Codeforces blocks automatic page reading. Paste the complete statement, including input/output and constraints, then submit again.</span><textarea value={problemStatement} onChange={(event) => setProblemStatement(event.target.value)} placeholder="Paste the Codeforces problem statement here if the page could not be loaded." rows={6} /></label></form>{error && <p role="alert" className="error-banner">{error}</p>}</section>
    {restoringSession ? <section className="mx-auto max-w-[1120px] px-5 py-16 text-slate-400 md:px-8">Restoring your active tutoring session…</section> : !session ? <EmptyState /> : <section className="mx-auto grid max-w-[1600px] gap-5 px-5 pb-8 md:grid-cols-[minmax(0,1fr)_minmax(390px,0.9fr)] md:px-8">
      <article className="panel problem-panel"><div className="panel-head"><div><p className="eyebrow">Problem context</p><h1>{session.state.problemData.title}</h1></div><a href={session.state.problemData.url} target="_blank" rel="noreferrer">Open on CF ↗</a></div><div className="meta-row"><span>{session.state.problemData.difficulty ? `${session.state.problemData.difficulty} rating` : "Unrated"}</span><span>{session.state.problemData.timeLimit}</span><span>{session.state.problemData.memoryLimit}</span></div><div className="tag-row">{session.state.problemData.tags.length ? session.state.problemData.tags.map((tag) => <span key={tag}>{tag}</span>) : <span>Tags unavailable</span>}</div><div className="reading problem-reading scrollbox"><h2>Statement <span className="source-badge">{session.state.problemData.statementSource === "user-pasted" ? "Pasted fallback" : "Codeforces"}</span></h2><p>{session.state.problemData.statement}</p><h2>Constraints clue</h2><p>{session.state.problemData.constraints}</p></div></article>
      <aside className="space-y-5 mentor-column"><article className="panel strategy-panel scrollbox"><div className="panel-head"><div><p className="eyebrow">Strategy map</p><h2>{session.state.requiredPattern}</h2></div></div><p className="strategy">{session.state.strategy}</p></article><MentorChat turns={session.state.turns ?? []} isSolved={session.state.isSolved} busy={reviewPending} onReview={review} /></aside>
    </section>}</main>;
}
